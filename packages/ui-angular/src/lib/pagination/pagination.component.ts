...
export class PaginationLinkComponent {
  ...
  @Input() class = ''
  get linkClass() {
    return cn('inline-flex items-center justify-center rounded-md border text-sm h-11 w-11 hover:bg-accent', this.isActive ? 'bg-primary text-primary-foreground hover:bg-primary/90 border-primary' : 'bg-background', this.class)
  }
}

@Component({ selector: 'paul-pagination-previous', standalone: true, template: `<a class="inline-flex h-11 items-center gap-1 px-3 text-sm rounded-md border hover:bg-accent"><span>«</span><span>Previous</span></a>`, changeDetection: ChangeDetectionStrategy.OnPush })
export class PaginationPreviousComponent {}

@Component({ selector: 'paul-pagination-next', standalone: true, template: `<a class="inline-flex h-11 items-center gap-1 px-3 text-sm rounded-md border hover:bg-accent"><span>Next</span><span>»</span></a>`, changeDetection: ChangeDetectionStrategy.OnPush })
export class PaginationNextComponent {}

@Component({ selector: 'paul-pagination-ellipsis', standalone: true, template: `<span aria-hidden="true" class="flex h-11 w-11 items-center justify-center"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/></svg><span class="sr-only">More pages</span></span>`, changeDetection: ChangeDetectionStrategy.OnPush })
