...
export class SidebarComponent {
  ...
  get sideClass() {
    return `flex flex-col h-full w-64 border-e bg-sidebar text-sidebar-foreground print:hidden ${this.side === 'right' ? 'order-last border-s border-e-0' : ''}`
  }
}
...
@Component({
  ...
  standalone: true,
  template: `<button class="peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-start text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pe-8"><ng-content /></button>`,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
