...
export class DialogComponent implements OnDestroy {
  ...
  get panelClass() {
    return cn(
      'fixed start-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 border bg-background p-6 shadow-lg sm:rounded-lg animate-in fade-in-0 zoom-in-95 print:shadow-none print:rounded-none print:border-black print:text-black',
      this.class
    )
  }
  get closeClass() {
    return 'absolute end-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2'
  }
