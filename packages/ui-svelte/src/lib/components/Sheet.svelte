...
{#if open}
  <div class="fixed inset-0 z-50">
    ...
    <div class={['fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out', sideClass[side] ?? sideClass.right, className].join(' ')} {...props}>
        {@render children?.()}
        <button onclick={() => (open = false)} class="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2" aria-label="Close">
