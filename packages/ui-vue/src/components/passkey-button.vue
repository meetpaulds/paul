...
const isUnsupported = computed(() => props.status === 'unsupported')
const buttonLabel = computed(() => props.label ?? 'Sign in with passkey')
...
const message = computed(() => props.statusMessage ?? defaultMessages[props.status ?? ''] ?? '')

defineOptions({ name: 'PasskeyButton' })
</script>
